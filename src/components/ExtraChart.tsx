...
export function ExtraChart({ rows, seriesIds, title }: ExtraChartProps) {
  return (
    <section className="overflow-hidden rounded-none bg-[#ffffff] shadow-[0_10px_26px_rgba(23,33,29,0.09)]">
      <div className="flex items-center gap-2 border-b border-[#e4e3dc] px-4 py-3.5">
        <ChartSpline aria-hidden="true" className="text-[#8a6a26]" size={17} strokeWidth={1.9} />
      ...
      <div className="h-64 w-full px-2 pb-2 pt-4 sm:h-72 sm:px-4" role="img" aria-label={`${title} chart`}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={rows} margin={{ top: 4, right: 10, left: 0, bottom: 0 }}>
            ...
            <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: '#55605a', fontSize: 11 }} />
            <YAxis axisLine={false} tickLine={false} width={48} tick={{ fill: '#55605a', fontSize: 11 }} />
            <Tooltip
              contentStyle={{
                border: 0,
                borderRadius: 10,
