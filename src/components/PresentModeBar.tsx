...
export function PresentModeBar({ step, stepIndex, totalSteps, onPrev, onNext, onExit }: PresentModeBarProps) {
  return (
    <motion.div
      ...
      className="fixed inset-x-0 bottom-4 z-50 mx-auto flex w-fit max-w-[calc(100%-2rem)] items-center gap-3 rounded-full border border-[#0b0d0c] bg-[#0b0d0c] px-4 py-2.5 text-[#ffffff] shadow-[0_12px_30px_rgba(23,33,29,0.35)]"
    >
      <button
        type="button"
        onClick={onExit}
        aria-label="Exit present mode"
        className="grid size-8 shrink-0 place-items-center rounded-full text-[#c7c6bd] transition hover:bg-white/10 hover:text-white"
      >
        <X aria-hidden="true" size={16} strokeWidth={2} />
      </button>
      <span className="whitespace-nowrap text-xs font-semibold uppercase tracking-[0.1em] text-[#c7c6bd]">
