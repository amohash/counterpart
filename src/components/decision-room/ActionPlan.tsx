    ...
    return (
      <section className="rounded-none bg-[#ffffff] p-4 shadow-[0_10px_26px_rgba(23,33,29,0.09)]">
        <h2 className="mb-2 flex items-center gap-2 text-sm font-semibold tracking-[-0.01em]">
          <ListChecks aria-hidden="true" className="text-[#8a6a26]" size={16} strokeWidth={1.9} />
          30-day action plan
        </h2>
    ...
    <section className="rounded-none bg-[#ffffff] p-4 shadow-[0_10px_26px_rgba(23,33,29,0.09)]">
      ...
      <ul className="flex flex-col gap-2.5">
        {items.map((item) => {
          ...
          return (
            <li key={item.id} className="flex items-start gap-2.5 rounded-none border border-[#e4e3dc] bg-white p-3 text-xs">
              <input
                type="checkbox"
                checked={isComplete}
                onChange={() => onToggle(item.id, item.title, !isComplete)}
                className="mt-0.5 size-4 shrink-0 accent-[#8a6a26]"
                aria-label={`Mark "${item.title}" ${isComplete ? 'incomplete' : 'complete'}`}
              />
              <div className={isComplete ? 'line-through opacity-60' : undefined}>
                <p className="text-[10px] font-semibold uppercase tracking-[0.08em] text-[#8b928c]">
                  Week {item.week}
                </p>
                <p className="mt-0.5 text-sm font-semibold text-[#0b0d0c]">{item.title}</p>
                <p className="mt-0.5 leading-5 text-[#55605a]">{item.detail}</p>
              </div>
            </li>
          );
        })}
